...
class TestUnitTests:
    ...
    async def container_with_setup(self, context_for_connector_with_setup, current_platform):
        result = await BuildConnectorImage(context_for_connector_with_setup, current_platform).run()
    ...
    async def container_with_poetry(self, context_for_connector_with_poetry, current_platform):
        result = await BuildConnectorImage(context_for_connector_with_poetry, current_platform).run()
        return result.output_artifact
    ...
    async def test__run_for_setup_py(self, context_for_connector_with_setup, container_with_setup):
        ...
        pip_freeze_output = (
            await result.output_artifact.with_workdir("/test_environment").with_exec(["pip", "freeze"], skip_entrypoint=True).stdout()
        )
        assert (
    ...
    async def test__run_for_poetry(self, context_for_connector_with_poetry, container_with_poetry):
        ...
        pip_freeze_output = (
            await result.output_artifact.with_workdir("/test_environment")
            .with_exec(["poetry", "run", "pip", "freeze"], skip_entrypoint=True)
            .stdout()
        )
        assert (
